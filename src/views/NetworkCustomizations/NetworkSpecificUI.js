...
class NetworkSpecificUI extends Component {
  ...
  onSubmit () {
    ...
    // themselves.
    const subs = this.state.networkTypeNames
      .map(name => ({ name, component: this.subComponents[name] }))
      .filter(x => x.component && x.component.isChanged())
    const submitSub = x => x.component.onSubmit().then(
        identity,
        err => `Remote network update failure on ${x.name}: ${err}`
    )
    return Promise.all(subs.map(submitSub))
  }
